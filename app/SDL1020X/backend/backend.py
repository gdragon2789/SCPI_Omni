...
class SOURce_COMMON(Enum):
    ...
    SET_SOURce_FUNCtion_TRANsient = ":SOURce:FUNCtion:TRANsient {mode}"
    GET_SOURce_FUNCtion_TRANsient = ":SOURce:FUNCtion:TRANsient?"
    SET_SOURce_FUNCtion = ":SOURce:FUNCtion {mode}"
...
class SDL1020X(VISA_INSTRUMENT):
    ...
    def enable_output(self):
        cmd = SOURce_COMMON.SET_SOURce_INPUT_STATE.value.format(state=DeviceConst.ON.value)
        self.write(command=cmd)

        #check the status
        return self.device_validator(command=SOURce_COMMON.GET_SOURce_INPUT_STATE.value,
                              result=DeviceConst.ON.value)

    def disable_output(self):
        cmd = SOURce_COMMON.SET_SOURce_INPUT_STATE.value.format(state=DeviceConst.OFF.value)
        self.write(command=cmd)
    ...
    def static_cc_mode(self):
        cmd = SOURce_COMMON.SET_SOURce_FUNCtion.value.format(mode=DeviceConst.CURR.value)
        self.write(command=cmd)

    def static_cv_mode(self):
        cmd = SOURce_COMMON.SET_SOURce_FUNCtion.value.format(mode=DeviceConst.VOLT.value)
        self.write(command=cmd)

    def static_cr_mode(self):
        cmd = SOURce_COMMON.SET_SOURce_FUNCtion.value.format(mode=DeviceConst.RES.value)
        self.write(command=cmd)

    def static_cp_mode(self):
        cmd = SOURce_COMMON.SET_SOURce_FUNCtion.value.format(mode=DeviceConst.POW.value)
        self.write(command=cmd)

    def static_led_mode(self):
        cmd = SOURce_COMMON.SET_SOURce_FUNCtion.value.format(mode=DeviceConst.LED.value)
        self.write(command=cmd)

    def transient_cc_mode(self):
        cmd = SOURce_COMMON.SET_SOURce_FUNCtion_TRANsient.value.format(mode=DeviceConst.CURR.value)
        self.write(command=cmd)

    def transient_cv_mode(self):
        cmd = SOURce_COMMON.SET_SOURce_FUNCtion_TRANsient.value.format(mode=DeviceConst.VOLT.value)
        self.write(command=cmd)

    def transient_cr_mode(self):
        cmd = SOURce_COMMON.SET_SOURce_FUNCtion_TRANsient.value.format(mode=DeviceConst.RES.value)
        self.write(command=cmd)

    def transient_cp_mode(self):
        cmd = SOURce_COMMON.SET_SOURce_FUNCtion_TRANsient.value.format(mode=DeviceConst.POW.value)
        self.write(command=cmd)

    def enable_short_output(self):
        cmd = SOURce_COMMON.SET_SOURce_SHORt_STATE.value.format(state=DeviceConst.ON.value)
        self.write(command=cmd)

    def disable_short_output(self):
        cmd = SOURce_COMMON.SET_SOURce_SHORt_STATE.value.format(state=DeviceConst.OFF.value)
        self.write(command=cmd)

    def unit_test(self):
        test_list = [self.enable_output,
                     self.disable_output,
                     self.static_cc_mode,
                     self.static_cv_mode,
                     self.static_cr_mode,
                     self.static_cp_mode,
                     self.static_led_mode,
                     self.transient_cc_mode,
                     self.transient_cv_mode,
                     self.transient_cr_mode,
                     self.transient_cp_mode,
                     self.enable_short_output,
                     self.disable_short_output]
        start = time.time()
        try:
        ...
        print(f"Total time: {end - start}, "
              f"Average time for {len(test_list)} tests: {(end - start)/len(test_list)}")
